...
import type { DatasetColumn, ImportResult } from '@fastgpt/global/core/evaluation/type';

interface DatasetFormType {
  name: string;
  description?: string;
  dataFormat: 'csv' | 'json';
...
const DatasetModal: React.FC = () => {
  ...
  const {
    ...
    defaultValues: {
      ...
      columns: [
        ...
        { name: 'expectedResponse', type: 'string', required: true, description: 'Expected answer' }
      ]
    }
  });
  ...
  const onSubmit = async (data: DatasetFormType) => {
  ...
  const handleCreateAndUpload = async (data: DatasetFormType) => {
    if (!selectedFile) {
      await saveDataset(data);
      return;
    }
