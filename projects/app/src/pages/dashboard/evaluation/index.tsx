...
import TargetList from '@/components/evaluation/target/TargetList';
import TargetModal from '@/components/evaluation/target/TargetModal';
...
import { getTargetList } from '@/web/core/evaluation/target';
...
const Evaluation = () => {
  ...
  const { runAsync: loadTargets } = useRequest2(
    () => getTargetList({ pageNum: 1, pageSize: 100 }),
    { manual: true }
  );
  ...
  useEffect(() => {
    const hasRunningOrErrorTasks = tasks.some((item: evaluationType) => {
  ...
  useEffect(() => {
    ...
    loadTargets();
    loadMetrics();
  }, [loadDatasets, loadTargets, loadMetrics]);
  ...
  const { runAsync: onRestartTask } = useRequest2(
    (evaluationId: string) => restartEvaluation(evaluationId),
    {
      onSuccess: () => {
        fetchTaskData();
        toast({
  ...
  const evalDetail = tasks.find((item: evaluationType) => item._id === evalDetailId);

  // 判断任务状态的帮助函数
  const getTaskStatus = (item: evaluationType) => {
          ...
          <Flex h={'full'} bg={'white'} p={6} flexDirection="column">
            ...
            <Tabs index={activeTab} onChange={setActiveTab}>
              <TabList>
                ...
                <Tab>{t('dashboard_evaluation:targets')}</Tab>
              ...
              <TabPanels>
                ...
                <TabPanel px={0}>
                  <TargetList searchKey={searchKey} onSearchChange={setSearchKey} />
                </TabPanel>
                <TabPanel px={0}>
