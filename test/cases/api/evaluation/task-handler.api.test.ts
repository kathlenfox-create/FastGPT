...
import { handler as restartHandler } from '@/pages/api/core/evaluation/task/restart';
...
describe('Task API Handler Tests (Direct Function Calls)', () => {
  ...
  describe('Create Evaluation Handler', () => {
    test('应该成功创建评估任务', async () => {
      const mockReq = {
        ...
        body: {
          ...
          metricIds: [new Types.ObjectId().toString()]
        }
      } as any;
      ...
      expect(EvaluationTaskService.createEvaluation).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          targetId: mockReq.body.targetId,
          metricIds: mockReq.body.metricIds
        }),
        expect.objectContaining({
          req: mockReq,
          authToken: true
    ...
    test('应该拒绝空指标列表', async () => {
      ...
      await expect(createHandler(mockReq)).rejects.toMatch('At least one metric is required');
    });
    ...
    test('应该拒绝缺少必填字段', async () => {
      const mockReq = {
        ...
        body: {
          name: 'Test Evaluation'
          // 缺少 datasetId, targetId, metricIds
        }
      } as any;

      await expect(createHandler(mockReq)).rejects.toMatch('Dataset ID is required');
    });
  });
  ...
  describe('Restart Evaluation Handler', () => {
    test('应该成功重启评估任务', async () => {
      const evaluationId = new Types.ObjectId().toString();
      const mockReq = {
        method: 'POST',
        body: { evaluationId }
      ...
      (EvaluationTaskService.restartEvaluation as any).mockResolvedValue(undefined);

      const result = await restartHandler(mockReq);

      expect(EvaluationTaskService.restartEvaluation).toHaveBeenCalledWith(
        evaluationId,
        expect.objectContaining({
          req: mockReq,
          authToken: true
      ...
      expect(result).toEqual({ message: 'Evaluation restarted successfully' });
    });
  });
