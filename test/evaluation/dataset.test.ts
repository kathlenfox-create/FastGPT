...
describe('EvaluationDatasetService', () => {
  ...
  describe('createDataset', () => {
    test('应该成功创建数据集', async () => {
      const params: CreateDatasetParams = {
        ...
        dataFormat: 'csv',
        columns: [
          { name: 'question', type: 'string', required: true, description: '测试问题' },
          { name: 'expectedResponse', type: 'string', required: true, description: '期望回答' },
  ...
  describe('getDataset', () => {
    test('应该成功获取数据集', async () => {
      ...
      const params: CreateDatasetParams = {
        ...
        dataFormat: 'csv',
        columns: [{ name: 'question', type: 'string', required: true, description: '测试问题' }]
      };
  ...
  describe('updateDataset', () => {
    test('应该成功更新数据集', async () => {
      ...
      const updates = {
        ...
        columns: [
          { name: 'question', type: 'string' as const, required: true },
          { name: 'expectedResponse', type: 'string' as const, required: true }
        ]
      };
  ...
  describe('validateDataFormat', () => {
    ...
    test('应该验证有效数据', async () => {
      const validData = [
        { question: 'What is AI?', expectedResponse: 'Artificial Intelligence', score: 95 },
    ...
    test('应该检测缺少必填字段', async () => {
      const invalidData = [
        { question: 'What is AI?' }, // 缺少 expectedResponse
        { expectedResponse: 'Machine Learning' } // 缺少 question
      ];
      ...
      expect(result.errors.some((error) => error.includes('expectedResponse'))).toBe(true);
      expect(result.errors.some((error) => error.includes('question'))).toBe(true);
    });

    test('应该检测数据类型错误', async () => {
      const invalidData = [
        { question: 'What is AI?', expectedResponse: 'Artificial Intelligence', score: 'high' } // score 应该是数字
      ];
    ...
    test('应该处理未知字段', async () => {
      const dataWithUnknownFields = [
        { question: 'What is AI?', expectedResponse: 'AI', unknownField: 'value' }
      ];
  ...
  describe('importData', () => {
    test('应该成功导入有效数据', async () => {
      ...
      const testData: DatasetItem[] = [
        { question: 'Test question 1', expectedResponse: 'Test answer 1' },
        { question: 'Test question 2', expectedResponse: 'Test answer 2' }
      ];
    ...
    test('应该拒绝无效数据', async () => {
      const invalidData = [
        { question: 'Question without answer' } // 缺少必填字段
      ];
  ...
  describe('exportData', () => {
    test('应该成功导出 JSON 格式数据', async () => {
      ...
      // 导入一些数据
      const testData: DatasetItem[] = [
        { question: 'Test question', expectedResponse: 'Test response' },
        { question: 'Test question 2', expectedResponse: 'Test response 2' }
      ];
    ...
    test('应该成功导出 CSV 格式数据', async () => {
      ...
      // 导入一些数据
      const testData: DatasetItem[] = [
        { question: 'Test question CSV', expectedResponse: 'Test response CSV' }
      ];
      await EvaluationDatasetService.importData(created._id, testData, auth);
      ...
      expect(csvContent.includes('question,expectedResponse')).toBe(true);
      expect(csvContent.includes('Test question CSV')).toBe(true);
    });
  });
